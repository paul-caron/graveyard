...
int main(int argc, char ** argv){
    string text = "Hello, OpenSSL encryption!";
    cout << "Original Text: " << text << std::endl;

    auto encryptedText = encrypt(text, key, iv);
    auto decryptedText = decrypt(encryptedText, key, iv);
...
void getPasswords(){
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW){
        ...
        auto decryptedText = decrypt(encrypted, key, iv);
        cout << "Tag: " << tag << ", username: " << credentialName << ", password: " << decryptedText << endl;
    }
